...
class RiskEnvFlat(gym.Env):
    def __init__(self, bot_types=None, env_config=None):
        ...
        self.invalid_move_penalty = -0.05 #less than survival bonus
        ...
        self.continents, self.territories, self.players = create_board_test(self.num_players, self.bot_types)
        self.turns_passed = 0
        self.phase = 0
                    ...
                    if not self.skip_bots:
                        for p in self.players[1:]:
                            if p.is_bot and p.territory_count > 0:
                                assert(p.make_move(self.players, self.territories, True) == True) #only allow legal moves from the other players
